...
```{r Fig 1 using stat sum and no stupid error bar code}


ggplot(data = patchrecovFinal, ) +
 stat_summary(aes(x=Date, y = per.final, lty = Treatment), geom = "line", fun.data = "mean_se") +
  stat_summary(aes(x=Date, y = per.final, shape = Treatment), geom = "pointrange", fun.data = "mean_se") +
...
patchrecov_mer <- lmer(per.final ~ Date*Treatment* (1|Site) *(1|Plot), REML = T , data = patchrecovFinal %>% filter(!Date == "2015-12-17"))
